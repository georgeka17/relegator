...
train_ds = np_to_tfds(X_train, y_train, batch_size=len(X_train)) #numpy to tensorflow dataset.
...
if 'write_results' in sys.argv:
    ...
    write_header = not os.path.exists('./fit_results/fit_results_model_architecture.txt')
    print('\nwriting results to file ' + './fit_results/fit_results_model_architecture.txt')
    f = open('./fit_results/fit_results_model_architecture.txt', 'a+')
    if write_header:
